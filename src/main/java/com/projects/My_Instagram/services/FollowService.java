...
public class FollowService {
    ...
    public List<UserResponse> getFollowersOf(String username) {
        User targetUser = userUtils.fetchUser(username);
        User currentUser = userUtils.fetchCurrectUser();

        if (isValidUser(targetUser, currentUser)) {
            throw new AppException(ExceptionMessages.UNAUTHORIZED.getMessage(), HttpStatus.UNAUTHORIZED);
        }
    ...
    public List<UserResponse> getFollowingOf(String username) {
        User targetUser = userUtils.fetchUser(username);
        User currentUser = userUtils.fetchCurrectUser();
        ...
        List<UserResponse> following = new ArrayList<>();
        for (User user : targetUser.getFollowing()) {
            following.add(Helper.formUserResponse(user));
        }

        return following;
    }

    private static boolean isValidUser(User targetUser, User currentUser) {
        return Boolean.TRUE.equals(targetUser.getPrivateAccount())
